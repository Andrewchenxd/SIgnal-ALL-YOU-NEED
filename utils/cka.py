    ...
    def calculate_cka_matrix(self) -> torch.Tensor:
        ...
        for epoch in range(self.num_epochs):
            ...
            for it, (imgs, *_) in enumerate(loader):
                ...
                all_layer_X = [x for x in all_layer_X if x.shape == (64, 64)]
                all_layer_Y = [x for x in all_layer_Y if x.shape == (64, 64)]
                # all_layer_X = [i for j, i in enumerate(all_layer_X) if j not in [6, 8, 11, 13, 21, 22]]
                # all_layer_Y = [i for j, i in enumerate(all_layer_Y) if j not in [6, 8, 11, 13, 21, 22]]
