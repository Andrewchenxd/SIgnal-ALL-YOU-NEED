...
class PatchShuffle(torch.nn.Module):
    ...
    def forward(self, patches_noise: torch.Tensor):
        ...
        return patches_noise, forward_indexes, backward_indexes
...
class MAE_Encoder(torch.nn.Module):
    def __init__(self,
        ...
        self.transformer = torch.nn.Sequential(
            *[Block(emb_dim, num_head, proj_drop=0.2, attn_drop=0.2) for _ in range(num_layer)])
        if self.is_attn:
            self.attn = CrossAttention_VIT(dim=emb_dim, L=int((image_size // patch_size) ** 2 * (1 - mask_ratio) + 1),
                                           attn_drop=0.2)
    ...
    def forward(self, noise):
        ...
        patches_noise, forward_indexes, backward_indexes = self.shuffle(patches_noise)
        ...
        return features_noise, backward_indexes
...
class MAE_ViT_gasf(torch.nn.Module):
    ...
    def forward(self, clean):
        features_clean, backward_indexes = self.encoder(clean)
