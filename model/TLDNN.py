...
class TransformerLSTM(nn.Module):
    def __init__(self, classes: int, d_model: int = 64, nhead: int = 8, d_hid: int = 128, lstm_hidden_size: int = 64,
        ...
        self.transformer_encoder = nn.Sequential(
            Transformer(dim=d_model, depth=nlayers, heads=nhead, dim_head=d_model // nhead, mlp_dim=d_hid,
                        talk_heads=True, dropout=dropout),
        )

        self.lstm_layer = nn.LSTM(input_size=d_model, hidden_size=lstm_hidden_size, num_layers=4, dropout=dropout,
                                  batch_first=True)
...
if __name__ == '__main__':
    ...
    num_params = count_parameters_in_MB(net)
